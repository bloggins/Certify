...
namespace Certify.Lib
{
    class DisplayUtil
    {
        ...
        public static void PrintCertificateInfo(X509Certificate2 ca)
        {
            ...
            names.Reverse();
            Console.WriteLine(new string("    Preg Punva                    : {0}".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), String.Join(new string(" -> ".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), names));
        }
