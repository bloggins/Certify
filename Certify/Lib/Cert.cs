        ...
        public static int SendCertificateRequest(string CA, string message)
        {
            ...
            switch (iDisposition)
            {
                ...
                case CR_DISP_UNDER_SUBMISSION:
                    ...
                    break;
                default:
                    Console.WriteLine(new string("\\e\a[!] PN Erfcbafr             : Gur fhozvffvba snvyrq: {0}".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), objCertRequest.GetDispositionMessage());
                    Console.WriteLine(new string("[!] Ynfg fgnghf             : 0k{0:K}".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), (uint)objCertRequest.GetLastStatus());
                    break;
            }
