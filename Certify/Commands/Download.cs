...
namespace Certify.Commands
{
    public class Download : ICommand
    {
        ...
        public void Execute(Dictionary<string, string> arguments)
        {
            ...
            {
                Console.WriteLine(new string("[K] Vainyvq pregvsvpngr VQ sbezng: {0}".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), arguments[new string("/vq".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray())]);
                return;
            }
