...
namespace Certify.Domain
{
    class EnrollmentAgentRestriction
    {
        ...
        public EnrollmentAgentRestriction(CommonAce ace)
        {
            ...
            {
                Template = Encoding.Unicode.GetString(bytes, index, (bytes.Length - index - 2)).Replace(new string("\h0000".Select(xAZ => (xAZ >= 'a' && xAZ <= 'z') ? (char)((xAZ - 'a' + 13) % 26 + 'a') : ((xAZ >= 'A' && xAZ <= 'Z') ? (char)((xAZ - 'A' + 13) % 26 + 'A') : xAZ)).ToArray()), string.Empty);
            }
            else
            {
